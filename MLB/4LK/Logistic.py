...
class LinearRegression():
    ...
    def __init__(self):
        ...
        self.epochs = 5
        self.reg_const = 0.05
        self.linear = torch.nn.Sequential(torch.nn.Linear(3*32*32,3*32*16),nn.ReLU(),torch.nn.Linear(3*32*16,3*32*8),nn.ReLU(),nn.Linear(3*32*8,10))
    ...
    def train(self, X_train, y_train):
        ...
        for i in range(0,10):
            for batch, label in zip(X_train, y_train):
                ...
                loss = loss_fn(output,label.squeeze(0))
                #optimizer.zero_grad()
                loss.backward()
                optimizer.step()
